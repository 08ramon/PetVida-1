...
BEGIN
    ...
    IF EXISTS (
        ...
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Horário já ocupado';
    END IF;
    ...
    VALUES (v_consulta_id, p_valor, 'pendente');
...
BEGIN
    ...
    IF EXISTS (
        ...
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Pagamento já está pago';
    END IF;
...
BEGIN
    ...
    SELECT LAST_INSERT_ID() AS animal_criado;
END $$
